...
const SetLocationModal = (): JSX.Element => {
  const [locationNames, setLocationNames] =
    useRecoilState(weatherLocationState);
  ...
  const uploadText = async () => {
    try {
      const response = await axios.post(
        "http://localhost:5175/weather/user-info",
        { cityName: cityName, locationName: locationName }
      );
        ...
        <div className="modal-box w-72">
          ...
          <form action="#" className="mt-4" onSubmit={onSubmitData}>
            ...
            <div className="flex flex-col gap-3">
              <label className="block text-sm text-gray-900 dark:text-white text-left">
                시/구/군
              </label>
              <input
                type="text"
