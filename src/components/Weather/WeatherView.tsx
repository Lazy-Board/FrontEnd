...
const WeatherView = (): JSX.Element => {
  ...
  const changeImg = weatherInformation.includes("비")
    ? "heavy-rain"
    : weatherInformation === "맑음"
    ? "sun"
    : weatherInformation === "눈"
    ? "snow"
    : weatherInformation === "안개"
    ? "foggy"
    : weatherInformation === "황사"
    ? "sand"
    : weatherInformation === "흐림"
    ? "cloudy"
    : weatherInformation === "우박"
    ? "hail"
    : weatherInformation === "번개"
    ? "thunder"
    : "moon";
    ...
    <div className="w-full h-fit mt-4 p-3 relative flex flex-wrap justify-between items-center border border-slate-300 rounded-lg bg-white">
      ...
      <TodayTemp>
        ...
        <p className="text-sm">
          ...
          <span className="ml-2 text-xs text-slate-400">{updatedAt}</span>
        </p>
        <div className="flex mt-1 items-baseline">
          <img
            src={`/weatherImage/${changeImg}.png`}
            width={40}
            height={40}
            alt=""
          />
