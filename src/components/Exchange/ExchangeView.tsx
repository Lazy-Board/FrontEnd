...
const ExchangeView = (): JSX.Element => {
  ...
  return (
    <div className="w-full h-fit mt-5 p-3 pt-2 pb-6 relative border border-slate-300 rounded-lg overflow-hidden bg-white">
      ...
      {0 < likedList.length ? (
        <LongWidth
          className="flex relative gap-3 overflow-hidden cursor-pointer"
          ref={slideRef}
        >
          {likedList.map((item: any) => (
            <div
              className="w-48 p-2 mt-2 text-left border border-slate-300 rounded-lg select-none"
            ...
            >
              ...
              <p className="text-2xl font-semibold">
                {item.tradingStandardRate}
              </p>
              <div
                className={`flex mt-2 items-center ${
                  item.comparedPreviousDay.includes("▼")
                    ? "text-blue-600"
                    : "text-red-500"
                }`}
              >
      ...
      <DotPosition
        className={`absolute flex bottom-0 z-50 ${
          3 > likedList.length ? "hidden" : ""
        }`}
      >
        {dots.map((dot) => (
          <button
